...
def createApp (connect):
    appname = "userbot" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def oturumacvebotlogolustur (stri, aid, ahash):
    try:
        ...
        await Client.start()
        ms = await Client.send_message('me',LANG['MAJESTEUSERBOT'])
        KanalId = await Client(CreateChannelRequest(
            title='UserBot BotLog',
...
if __name__ == "__main__":
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/HerlockBots/UserBot"
